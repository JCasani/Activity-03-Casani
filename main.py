...
if opt == 1:
    ...
    values()
    print(stats_core[stat_inp-1],"-->", stats1.userstat.stat(stat_inp,base,IV,EV,lvl,natures), end="\n\n")
    out()
    ...
    if(opt_1 == 1):
        ...
        print("EV Needed on",stats_core[stat_inp-1].upper(),"-->", userev.userev.single(stat_inp,base,IV,EV,lvl,natures,stat), end="\n\n")
    elif(opt_1 == 2): 
        print("\nALL STAT")
        ...
        for c in range (len(stats_core)):
            print(stats_core[c],userev.userev.all(desire,arr[c],desire[c],IV,EV,lvl,natures,stat), end="\n")
        print("\n")
    else: error1()
    out()
